...
export function initDatabase(): Database.Database {
  ...
  const dbPath = path.join(app.getPath('userData'), 'pokemon-tcg.db')
  ...
  // Check if ptcgo_code column exists before adding it
  const cols = db.prepare(`PRAGMA table_info(collections)`).all() as Array<{ name: string }>
  const hasPtcgoCode = cols.some((c) => c.name === 'ptcgo_code')
  console.log(`[db] ptcgo_code column exists: ${hasPtcgoCode}`)
  if (!hasPtcgoCode) {
    db.exec(`ALTER TABLE collections ADD COLUMN ptcgo_code TEXT`)
    console.log(`[db] ptcgo_code column added`)
  }
